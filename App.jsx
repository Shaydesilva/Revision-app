...
body{background:${BG};overscroll-behavior:none;font-family:${FONT}}
...
const SEED=[
  ...
  mk(26,"a gente se ve","we'll see each other","frase_pronta"),mk(27,"a gente","us / we — replaces nos in Carioca","grammar",{contrast:"nos"}),
  mk(28,"vamo pra praia","let's go to the beach","sentence",{cluster:"letsgo",contrast:"nos vamos a praia",scenario:"social"}),
  mk(29,"eu me mudei pro Rio","I moved to Rio","sentence",{contrast:"eu me mudei para o Rio"}),
  mk(30,"pra / pro","to the — contracted from para a / para o","grammar",{contrast:"para a / para o"}),
  ...
  mk(33,"me ve uma gelada","can I have a cold one","frase_pronta",{contrast:"eu gostaria de uma cerveja gelada",scenario:"ordering"}),
  ...
  mk(41,"vou pagar no credito","I'll pay by card","frase_pronta",{scenario:"shopping"}),mk(42,"eu tambem","me too","vocab"),
  mk(43,"sem gas","still water","vocab",{scenario:"ordering"}),mk(44,"exatamente","exactly","vocab"),mk(45,"concordo","I agree","vocab"),
  mk(46,"mesmo","same / really / even","vocab"),mk(47,"e mesmo","oh yeah, that's true","giria"),mk(48,"parecido","similar","vocab"),
  mk(49,"sem graca","boring / bland","vocab"),mk(50,"nasci e cresci no Rio","I was born and raised in Rio","sentence"),
...
async function dbLoad(uid){if(!sb)return null;try{const[{data:cards},{data:state}]=await Promise.all([sb.from('cards').select('*').eq('user_id',uid),sb.from('user_state').select('*').eq('user_id',uid).single()]);return{cards:(cards||[]).map(fromRow),state:state||null}}catch(e){console.error(e);return null}}
async function dbSeed(uid,cards){if(!sb)return;await sb.from('cards').upsert(cards.map(c=>toRow(c,uid)),{onConflict:'id,user_id'})}
async function dbUpdateCard(uid,card){if(!sb)return;await sb.from('cards').update({mastery:card.mastery,ease_factor:card.easeFactor,interval:card.interval,reps:card.reps,next_review:card.nextReview,sentence_score:card.sentenceScore||0,sentence_count:card.sentenceCount||0,updated_at:new Date().toISOString()}).eq('id',card.id).eq('user_id',uid)}
async function dbInsertCards(uid,cards){if(!sb)return;await sb.from('cards').insert(cards.map(c=>toRow(c,uid)))}
async function dbSaveState(uid,fluency,streak,lastDate,history){if(!sb)return;await sb.from('user_state').upsert({user_id:uid,fluency_rating:fluency,streak_days:streak,last_session_date:lastDate,sentence_history:history||[],updated_at:new Date().toISOString()},{onConflict:'user_id'})}
async function getSession(){if(!sb)return null;const{data}=await sb.auth.getSession();return data?.session||null}
async function sendMagicLink(email){if(!sb)return{error:{message:'No Supabase configured'}};return sb.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin}})}
function onAuthChange(cb){if(!sb)return()=>{};const{data}=sb.auth.onAuthStateChange((_,s)=>cb(s));return()=>data.subscription.unsubscribe()}

// Claude via Netlify proxy
async function callClaude(system,messages,maxTokens=900){const r=await fetch('/.netlify/functions/claude',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({system,messages,max_tokens:maxTokens})});const d=await r.json();return d.content?.[0]?.text||''}
const ct=(system,text,max)=>callClaude(system,[{role:'user',content:text}],max)

async function generateScenario(cards,history){
  const vocab=cards.filter(c=>c.mastery>=2).map(c=>`${c.portuguese} (${c.english}, mastery:${c.mastery}, sentenceUses:${c.sentenceCount})`).join('\n')
  ...
  const raw=await ct(`You are a Carioca Portuguese tutor in Rio creating sentence practice scenarios.
RULES: Create ONE realistic Rio situation. Student writes what they'd say — not a translation.
Use words the student knows (mastery>=2). Prioritise words not yet used in sentences: ${priority||'any'}.
Do NOT repeat: ${recent||'none'}. Set in real Rio life: boteco, praia, uber, rua, vizinho, mercado etc.
Accents optional — never penalise. Accept all Carioca speech as correct.
Reply ONLY valid JSON: {"english":"situation","targetWords":["w1","w2"],"scenario":"boteco|praia|rua|social|compras","context":"brief scene"}`,
  ...
  catch{return{english:"You just walked into your local boteco. Order a cold beer.",targetWords:["me ve","gelada"],scenario:"boteco",context:"At a bar in Rio"}}
}

async function evalAnswer(scenario,answer,cards){
  const raw=await ct(`You are a warm Carioca Portuguese tutor.
CRITICAL: COMPLETELY IGNORE accent marks. ó=o, ã=a, é=e etc.
Accept ALL Carioca speech as correct: tamo=estamos, pra=para, ta=esta, num=nao, ce=voce.
Score on MEANING and NATURALNESS only. Be encouraging but specific.
Reply ONLY valid JSON: {"wordScores":{"word":1-5},"naturalness":0-100,"feedback":"one line warm and specific","correction":"natural Carioca version or null"}`,
  ...
  }catch{return{wordScores:{},naturalness:50,feedback:"Couldn't evaluate — check connection.",correction:null,cardUpdates:{}}}
}

async function extractFromPDF(b64,existing){
  try{
    const raw=await callClaude(`Extract ALL vocabulary, phrases, expressions and grammar from this Brazilian Portuguese lesson document.
Return ONLY a JSON array — no markdown: [{"portuguese":"...","english":"..." or null,"type":"giria"|"vocab"|"frase_pronta"|"grammar"|"sentence","cluster":"..." or null,"contrast":"..." or null}]
cluster: give synonyms the same string. contrast: formal Portuguese equivalent if Carioca expression.`,
...
async function openChatAPI(scenarioType){
  const raw=await ct(`You are a Carioca local in Rio doing language practice. Speak naturally — short, casual, real Carioca contractions and giria.
Reply ONLY valid JSON: {"message":"opening in Portuguese","translation":"English translation","description":"brief situation in English"}`,
  `Scenario: ${scenarioType}\n\nOpen the conversation:`,300)
  try{return JSON.parse(raw.replace(/```json|```/g,'').trim())}
  catch{return{message:"Oi! Tudo bom?",translation:"Hey! All good?",description:"Casual greeting"}}
}

async function replyChatAPI(history){
  const convo=history.map(h=>`${h.role==='user'?'Student':'Carioca'}: ${h.content}`).join('\n')
  const raw=await ct(`You are a Carioca local having a casual conversation with a language student. Keep replies short, natural, Carioca. Use contractions and giria naturally.
Reply ONLY valid JSON: {"message":"reply in Portuguese","translation":"English translation"}`,
  `Conversation:\n${convo}\n\nYour reply:`,300)
  try{return JSON.parse(raw.replace(/```json|```/g,'').trim())}
  catch{return{message:"E mesmo?",translation:"Oh really?"}}
}
...
async function evalChatAPI(history,cards){
  ...
  const raw=await ct(`Evaluate this Portuguese conversation by a language student. IGNORE all accents. Accept all Carioca speech as correct. Focus on communication, vocabulary use, naturalness.
Reply ONLY valid JSON: {"overallScore":0-100,"feedback":"2-3 sentences warm specific feedback","corrections":["fix1"],"wordsUsedWell":["word1"]}`,
  `Student vocab: ${vocab}\n\nConversation:\n${convo}`,400)
  try{return JSON.parse(raw.replace(/```json|```/g,'').trim())}
...
function PBtn({label,onClick,disabled,full=true}){return <button onClick={disabled?null:onClick} style={{width:full?'100%':undefined,background:disabled?S2:AC,color:disabled?MU:'#fff',border:'none',borderRadius:13,padding:'15px 24px',fontSize:15,fontWeight:700,cursor:disabled?'not-allowed':'pointer',opacity:disabled?0.5:1,transition:'all 0.15s',fontFamily:FONT}} onMouseDown={e=>{if(!disabled)e.currentTarget.style.transform='scale(0.97)'}} onMouseUp={e=>e.currentTarget.style.transform='scale(1)'}>{label}</button>}
...
function Login(){
  ...
  const send=async()=>{if(!email.trim())return;setLoading(true);setErr('');const{error}=await sendMagicLink(email.trim());if(error){setErr(error.message);setLoading(false)}else setSent(true)}
  return <div style={{background:BG,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32}}>
    <div style={{width:'100%',maxWidth:360,animation:'up 0.4s ease'}}>
      <div style={{textAlign:'center',marginBottom:52}}>
        <div style={{fontSize:13,color:AC,letterSpacing:3,fontWeight:700,marginBottom:12}}>CARIOCA</div>
        <div style={{fontSize:30,color:TX,fontWeight:800,marginBottom:8,letterSpacing:-0.5}}>Learn Rio Portuguese</div>
        <div style={{fontSize:15,color:MU}}>Science-backed. Carioca-first.</div>
      </div>
      {!sent?<>
        <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} type="email" placeholder="your@email.com"
          style={{width:'100%',background:S,border:`1px solid ${BD}`,borderRadius:13,padding:'16px',color:TX,fontSize:16,outline:'none',marginBottom:12,boxSizing:'border-box'}}
          onFocus={e=>e.target.style.borderColor=AC} onBlur={e=>e.target.style.borderColor=BD}/>
        {err&&<div style={{color:RE,fontSize:13,marginBottom:10,textAlign:'center'}}>{err}</div>}
        <PBtn label={loading?'Sending…':'Send Magic Link'} onClick={send} disabled={!email.trim()||loading}/>
        <div style={{fontSize:12,color:MU,textAlign:'center',marginTop:16,lineHeight:1.7}}>No password needed. We'll email you a sign-in link.</div>
      </>:<div style={{textAlign:'center'}}>
        <div style={{fontSize:48,marginBottom:16}}>📬</div>
        <div style={{fontSize:20,fontWeight:700,color:TX,marginBottom:8}}>Check your email</div>
        <div style={{fontSize:14,color:MU,lineHeight:1.7}}>Sent to <span style={{color:TX}}>{email}</span>.<br/>Tap the link to sign in.</div>
      </div>}
    </div>
  </div>
}
...
function Home({cards,fluency,streak,lastDate,tier,go}){
  ...
  return <div style={{padding:'52px 24px 100px',animation:'up 0.35s ease'}}>
    ...
    {streak>0&&!studiedToday&&<div style={{marginTop:20,padding:'14px 18px',background:`${YE}15`,border:`1px solid ${YE}44`,borderRadius:14,display:'flex',alignItems:'center',gap:10}}>
      <span style={{fontSize:18}}>🔥</span><span style={{fontSize:13,color:YE}}>Study today to keep your {streak}-day streak</span>
    </div>}
  </div>
}
...
function Study({cards,onRate,onBack}){
  ...
  const advance=useCallback(q=>{onRate(card.id,q);setCombo(c=>q>=3?c+1:0);setHist(h=>[...h,{card,q}]);if(idx+1>=deck.length){setDone(true);return}doFlip(()=>{setIdx(i=>i+1);setFlipped(false);setPhase('front');setAns('');setEv(null);setCardKey(k=>k+1)})},[card,idx,deck,onRate,doFlip])
  const tap=useCallback(async()=>{
    if(flipped||flipping)return
    if(isDeep&&phase==='front'){setPhase('typing');return}
    if(isDeep&&phase==='typing'){if(!ans.trim()){doFlip(()=>{setFlipped(true);setPhase('back')});return}setPhase('evaluating');const res=await evalAnswer({english:card.english,targetWords:[card.portuguese]},ans,[card]);doFlip(()=>{setEv(res);setFlipped(true);setPhase('back')});return}
    ...
    <div style={{flex:1,padding:'8px 20px 16px',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{position:'relative',flex:1,display:'flex',flexDirection:'column'}}>
        ...
        <div key={cardKey} style={{position:'relative',flex:1,zIndex:2,display:'flex',flexDirection:'column',animation:'up 0.28s ease',opacity:flipping?0:1,transform:flipping?'scaleX(0.05)':'scaleX(1)',transition:flipping?'all 0.15s ease':'none'}}>
          {!flipped?<div onClick={phase!=='typing'?tap:undefined} style={{flex:1,background:S,border:`1px solid ${BD}`,borderRadius:22,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'32px 28px',textAlign:'center',cursor:phase==='typing'?'default':'pointer'}}>
          ...
          </div>:<div style={{flex:1,background:S,border:`1px solid ${BD}`,borderRadius:22,padding:'24px',overflowY:'auto',display:'flex',flexDirection:'column'}}>
            ...
            {ev&&<div style={{background:S2,borderRadius:14,padding:'14px',marginBottom:14}}>
              <div style={{display:'flex',gap:8,marginBottom:12}}>{[{l:'Accuracy',v:ev.naturalness||50},{l:'Carioca',v:ev.naturalness||50}].map(x=>{const c=x.v>=75?GR:x.v>=50?YE:RE;return <div key={x.l} style={{flex:1,background:BG,borderRadius:11,padding:'12px',textAlign:'center'}}><div style={{fontSize:24,fontWeight:800,color:c}}>{x.v}</div><div style={{fontSize:10,color:MU,fontWeight:600,marginTop:3}}>{x.l.toUpperCase()}</div></div>})}</div>
...
function StudyDone({hist,combo,onBack}){
  ...
  return <div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'60px 24px 100px',animation:'up 0.4s ease'}}>
    ...
    <div style={{display:'flex',gap:28,marginBottom:32}}>{[{v:ok,c:GR,l:'correct'},{v:al,c:YE,l:'almost'},{v:no,c:RE,l:'again'}].map(x=><div key={x.l} style={{textAlign:'center'}}><div style={{fontSize:34,fontWeight:800,color:x.c}}>{x.v}</div><div style={{fontSize:11,color:MU,fontWeight:500}}>{x.l}</div></div>)}</div>
    <div style={{display:'flex',flexWrap:'wrap',gap:6,justifyContent:'center',marginBottom:36,maxWidth:360}}>{hist.map((h,i)=><span key={i} style={{padding:'4px 12px',borderRadius:8,background:`${h.q>=4?GR:h.q>=3?YE:RE}18`,color:h.q>=4?GR:h.q>=3?YE:RE,fontSize:12,fontWeight:600}}>{h.card.portuguese}</span>)}</div>
...
function Practice({cards,onRateMultiple,sentenceHistory,onSaveSentence}){
  const[phase,setPhase]=useState('idle');const[scenario,setScenario]=useState(null);const[ans,setAns]=useState('');const[ev,setEv]=useState(null);const[count,setCount]=useState(0)
  ...
  const submit=useCallback(async()=>{if(!ans.trim()||!scenario)return;setPhase('evaluating');const res=await evalAnswer(scenario,ans,cards);setEv(res);setCount(c=>c+1);onRateMultiple(res.cardUpdates||{});onSaveSentence({english:scenario.english,userAnswer:ans,scenario:scenario.scenario,date:new Date().toISOString()});setPhase('result')},[ans,scenario,cards,onRateMultiple,onSaveSentence])
  return <div style={{padding:'20px 20px 40px'}}>
    {phase==='idle'&&<div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',paddingTop:48,gap:16,animation:'up 0.3s ease'}}>
      ...
      <div style={{fontSize:14,color:MU,lineHeight:1.7,maxWidth:300}}>Claude creates a real Rio scenario using your vocabulary. You write what you'd actually say — no accent penalties.</div>
      <div style={{background:S,border:`1px solid ${BD}`,borderRadius:13,padding:'14px 20px',fontSize:13,color:MU}}>{readyWords} words ready for production practice</div>
...
function Chat({cards,onRateMultiple}){
  ...
  const start=useCallback(async scenario=>{setPhase('loading');const res=await openChatAPI(scenario.key);setHistory([{role:'bot',content:res.message,translation:res.translation}]);setPhase('chatting')},[])
  const send=useCallback(async()=>{
    ...
    if(newHist.filter(h=>h.role==='user').length>=4){
      ...
      if(res.wordsUsedWell?.length){const updates={};res.wordsUsedWell.forEach(word=>{const card=cards.find(c=>c.portuguese.toLowerCase().includes(word.toLowerCase()));if(card)updates[card.id]=4});onRateMultiple(updates)}
      setPhase('done')
  ...
  if(phase==='pick')return <div style={{padding:'20px',animation:'up 0.3s ease'}}>
    ...
    <div style={{fontSize:13,color:MU,marginBottom:20}}>Claude plays a Carioca local. Have a real conversation in 4 turns.</div>
    <div style={{display:'flex',flexDirection:'column',gap:10}}>{SCENARIOS.map(s=><button key={s.key} onClick={()=>start(s)} style={{background:S,border:`1px solid ${BD}`,borderRadius:16,padding:'18px',textAlign:'left',cursor:'pointer',fontFamily:FONT,transition:'border-color 0.15s'}} onMouseEnter={e=>e.currentTarget.style.borderColor=AC} onMouseLeave={e=>e.currentTarget.style.borderColor=BD}>
      <div style={{fontSize:16,fontWeight:700,color:TX,marginBottom:3}}>{s.label}</div>
      <div style={{fontSize:13,color:MU}}>{s.desc}</div>
    </button>)}</div>
  </div>
  if(phase==='loading')return <div style={{display:'flex',flexDirection:'column',alignItems:'center',paddingTop:80,gap:16}}><Spinner size={28}/><span style={{fontSize:14,color:MU}}>Starting conversation…</span></div>
...
function Import({cards,onImport,onBack}){
  ...
  const handleFile=async e=>{
    ...
    try{const b64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=()=>res(r.result.split(',')[1]);r.onerror=rej;r.readAsDataURL(file)});const items=await extractFromPDF(b64,existing);setPreview(items);setStage('preview');items.forEach((_,i)=>setTimeout(()=>setVisible(v=>v+1),i*80))}catch{setStage('idle')}
  }
  return <div style={{padding:'52px 24px 100px',animation:'up 0.35s ease'}}>
    ...
    {stage==='preview'&&<>
      ...
      <div style={{maxHeight:320,overflowY:'auto',marginBottom:16,display:'flex',flexDirection:'column',gap:8}}>{preview.slice(0,visible).map((item,i)=><div key={i} style={{background:S,border:`1px solid ${BD}`,borderRadius:13,padding:'13px 18px',animation:'up 0.25s ease'}}><div style={{fontSize:15,fontWeight:700,color:TX}}>{item.portuguese}</div><div style={{fontSize:13,color:MU,marginTop:2}}>{item.english||'—'}</div></div>)}</div>
...
export default function App(){
  const[cards,setCards]=useState([]);const[fluency,setFluency]=useState(1000);const[streak,setStreak]=useState(0);const[lastDate,setLastDate]=useState(null);const[sentenceHistory,setSentenceHistory]=useState([]);const[screen,setScreen]=useState('home');const[loaded,setLoaded]=useState(false);const[authReady,setAuthReady]=useState(false);const[session,setSession]=useState(null);const uidRef=useRef(null)
  ...
  useEffect(()=>{getSession().then(s=>{setSession(s);setAuthReady(true)});return onAuthChange(s=>{setSession(s);if(!s)setLoaded(false)})},[])
  useEffect(()=>{
    if(!authReady)return
    const load=async()=>{
      if(session?.user?.id){uidRef.current=session.user.id;const data=await dbLoad(session.user.id);if(data?.cards?.length){setCards(data.cards);setFluency(data.state?.fluency_rating||1000);setStreak(data.state?.streak_days||0);setLastDate(data.state?.last_session_date||null);setSentenceHistory(data.state?.sentence_history||[])}else{await dbSeed(session.user.id,SEED);setCards(SEED)}}else{setCards(SEED)}
      setLoaded(true)
    };load()
  },[authReady,session])
  const touchStreak=useCallback(()=>{const today=new Date().toISOString().slice(0,10);if(lastDate===today)return;const yesterday=new Date(Date.now()-86400000).toISOString().slice(0,10);const ns=lastDate===yesterday?streak+1:1;setStreak(ns);setLastDate(today);if(uidRef.current)dbSaveState(uidRef.current,fluency,ns,today,sentenceHistory)},[lastDate,streak,fluency,sentenceHistory])
  const onRate=useCallback((id,q)=>{setCards(prev=>prev.map(c=>{if(c.id!==id)return c;const u=sm2(c,q);const updated={...c,...u};if(uidRef.current)dbUpdateCard(uidRef.current,updated);return updated}));setFluency(f=>Math.max(500,f+(q>=4?8:q>=3?2:-4)));touchStreak()},[touchStreak])
  const onRateMultiple=useCallback(cardUpdates=>{if(!Object.keys(cardUpdates||{}).length)return;setCards(prev=>prev.map(c=>{const q=cardUpdates[c.id];if(!q)return c;const u=sm2(c,q);const sc=((c.sentenceScore||0)*(c.sentenceCount||0)+q)/((c.sentenceCount||0)+1);const updated={...c,...u,sentenceScore:Math.round(sc*10)/10,sentenceCount:(c.sentenceCount||0)+1};if(uidRef.current)dbUpdateCard(uidRef.current,updated);return updated}));setFluency(f=>Math.max(500,f+4));touchStreak()},[touchStreak])
  const onImport=useCallback(items=>{const newCards=items.map((it,i)=>mk(`imp-${Date.now()}-${i}`,it.portuguese,it.english||'—',it.type||'vocab',{cluster:it.cluster||null,contrast:it.contrast||null}));setCards(prev=>[...prev,...newCards]);if(uidRef.current)dbInsertCards(uidRef.current,newCards)},[])
  const onSaveSentence=useCallback(entry=>{const updated=[...(sentenceHistory||[]).slice(-49),entry];setSentenceHistory(updated);if(uidRef.current)dbSaveState(uidRef.current,fluency,streak,lastDate,updated)},[sentenceHistory,fluency,streak,lastDate])
  useEffect(()=>{if(!loaded||!uidRef.current)return;const t=setTimeout(()=>dbSaveState(uidRef.current,fluency,streak,lastDate,sentenceHistory),4000);return()=>clearTimeout(t)},[fluency,streak,lastDate,loaded,sentenceHistory])
  if(!authReady)return <div style={{background:BG,height:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}><Spinner size={28}/></div>
  if(!session)return <Login/>
  if(!loaded)return <div style={{background:BG,height:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16,fontFamily:FONT}}><Spinner size={28}/><span style={{color:MU,fontSize:14}}>Loading your deck…</span></div>
  const due=cards.filter(c=>new Date(c.nextReview)<=new Date()&&c.mastery>0).length
  const mastered=cards.filter(c=>c.mastery>=5).length
