...
async function generateSegment(sb,profile,scaffolds,mem,prevLines,station,segIndex){
  ...
  const res=await fetch('https://api.anthropic.com/v1/messages',{
    ...
    body:JSON.stringify({
      ...
      system:`You write Radio Carioca — a continuous comedy radio show. Two hosts:
...
~85% of language from KNOWN patterns, ~10% FRONTIER (stealth teaching), 5% glue.
...
FRONTIER (weave 1-2 in naturally): ${frontier.join(', ')||'none'}
SHOW BIBLE: ${showBible.slice(0,500)}
...
exports.handler=async(event)=>{
  ...
  try{
    ...
    const[{data:profile},{data:scaffolds},{data:mem}]=await Promise.all([
    ...
    if(action==='tune'){
      ...
      if(!opener){
        ...
        const seg=await generateSegment(sb,profile,scaffolds,mem,null,stationPrompt,0)
    ...
    if(action==='next'&&session_key){
      ...
      const seg=await generateSegment(sb,profile,scaffolds,mem,prev?.lines||null,stationPrompt,segment_index)
