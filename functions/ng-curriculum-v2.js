...
exports.handler=async(event)=>{
  ...
  try{
    ...
    const{data:done}=await sb.from('ng_brain_log').select('id').eq('user_id',UID).eq('process','curriculum_v2_2').limit(1)
    ...
    await sb.from('ng_brain_log').insert({user_id:UID,process:'curriculum_v2_2',importance:2,
